...
def main(model_type, dataset_dir, csv_filename, dataset_type, dataset_id, layer, num_classes, repeat=1, parse_data=True, num_procs=1):
	...
	for iteration in range(repeat):
		...
		for ex in csv_contents:
			ex['iad_path'] = os.path.join(dataset_dir, 'iad_{0}_{1}_{2}'.format(model_type, dataset_type, dataset_id), '{0}_{1}.npz'.format(ex['example_id'], layer))
